...
public class ProductController {
	...
	public ModelAndView product(Criteria cri, @RequestParam("name") String name,
			@RequestParam("orderBy") String orderBy, Model model, HttpSession session) {
		logger.info("ModelAndView메소드 실행");
		ModelAndView mav = new ModelAndView();
		PageMaker pageMaker = new PageMaker();
		HashMap<String, Object> hash = new HashMap<String, Object>();
		String cateCodeRef = "";// 上位商品のコード
		String cateCode = "";// 下位商品のコード
		int code;// コード
		String p;// ページナンバー
	...
	public ModelAndView productSearch(Criteria cri, @RequestParam("searchType") String searchType,
			@RequestParam("keyWord") String keyWord, @RequestParam("orderBy") String orderBy, Model model,
			HttpSession session) {
	...
	public List<ReplyVO> getReplyList(int productNum) {
	...
	public ArrayList<ProductVO> myProductMore(@ModelAttribute("startNum") int startNum, HttpSession session)
			throws Exception {
	...
	// 商品登録ページのメソッド＆商品登録するとき必要なカテゴリーのメソッド
	...
	public String productUpload(RedirectAttributes redirect, HttpSession session,
			@RequestParam MultipartFile productImg, String role, String category1, String category2, String category3,
			String productTitle, String productQty, String content, String productPrice) throws IOException, Exception {
		logger.info("productUpload메소드 실행");
		int price = Integer.parseInt(productPrice.replaceAll(",", ""));// 商品の価格処理
		int qty = Integer.parseInt(productQty.replaceAll(",", ""));// 商品の数量処理
		String userID = (String) session.getAttribute("userID");
		...
		if (category3 == null) {
			...
			productVO.setProductTitle("사이즈는" + category3 + ": " + productTitle);// 商品がファッションだったらタイトルにサイズをつける
		}
	...
	// 商品登録するときsummernoteで作成したコンテンツ処理のメソッド
	@ResponseBody
	...
	public ModelAndView myProductView(@RequestParam("n") int n, HttpSession session) {
		...
		ArrayList<CategoryVO> category = service.category();
		ProductVO productVO = service.productView(productNum);// 修正する商品
		ThumbnailVO thum = service.productShowThum(productNum);// 修正する商品のサムネイル
	...
	public String productModify(HttpServletRequest request, @RequestParam MultipartFile productImg,
		...
		boolean result = service.productModify(productVO);// 商品修正
		if (result) {
			return "redirect:/";
		} else {
	...
	public String productDelete(@RequestParam("n") int n) throws Exception {
		...
		boolean result = service.productDelete(productNum);// 商品削除
	...
	public int deleteCart(HttpSession session, @RequestParam(value = "check_box[]") List<String> chArr,
			CartListVO cart) {
		...
		if (userID != null) {
			...
			for (String i : chArr) {
				...
				result = service.deleteCart(cart);// カートの商品削除
				if (result != 1) {
	...
	public String order(RedirectAttributes redirect, HttpSession session, OrderVO orderVO) {
		...
		if (result1 == true && result2 == true) {
			service.cartAllDelete(userID);// すべてのカートリストの商品削除
	...
	public ModelAndView orderComplete(HttpSession session) {
		...
		List<OrderVO> orderList = service.orderList(userID);// 注文した商品
		List<OrderVO> refundOrderList = service.refundOrderList(hash);// 返品した商品
	...
	// 注文した明細書の詳しい商品の内容のメソッド
	@RequestMapping(value = "orderView", method = RequestMethod.GET)
	public ModelAndView orderView(@RequestParam("n") String orderID, @RequestParam("productInfo") String productInfo,
			HttpSession session) {
		...
		hash.put("productInfo", productInfo);// 商品の配送の状態に合わせて商品を出力する

		List<OrderVO> orderViewVO = service.orderView(hash);// 注文した商品の詳しい内容のリスト
	...
	public int refundPlz(Model model, @RequestParam String jsonData) {
		...
		for (int i = 0; i < array.size(); i++) {
			...
			result = service.refundPlz(hash);// orderViewからチェックした商品の返品要請
		}
		return result;
	}

	// 販売者：明細書の配送の状態に合わせて商品を持ってくるページ
	@RequestMapping(value = "delivery", method = RequestMethod.GET)
	public ModelAndView delivery(HttpSession session, Model model, @RequestParam("orderID") String orderID,
		...
		hash.put("productInfo", productInfo);// 商品の配送の状態に合わせて持ってくる

		List<OrderVO> orderDelivery = service.orderDelivery(hash);// 販売者：明細書の配送の状態に合わせて商品を持ってくる
	...
	public String deliveryInfo(HttpSession session, String delivery, String orderID) {
		...
		if (delivery.equals("배송완료")) {
			List<OrderVO> orderVO = service.orderView(hash);// 注文が入った商品を持ってきて
			ProductVO productVO = new ProductVO();

			for (OrderVO i : orderVO) {
				productVO.setProductNum(i.getProductNum());
				productVO.setProductQty(i.getProductQty());
				service.productDown(productVO);// 配送が完了すると注文が入った数分だけ商品から抜く
			}
		}
	...
	public int refund(Model model, @RequestParam String jsonData) {
		...
		int result = 0;
		  ...
		  for(int i=0; i<array.size(); i++) { 
			 JSONObject obj = (JSONObject)array.get(i); 
			 String numQty = (String) obj.get("productNum");
			 String array1 []; 
			 array1 = numQty.split(","); 
			 int productNum = Integer.parseInt(array1[0]); 
			 int productQty = Integer.parseInt(array1[1]);
			  
			 hash.put("productNum", productNum); 
			 hash.put("productQty", productQty);
			 hash.put("orderID", obj.get("orderID"));
			  
			 result = service.refund(hash);// 返品完了すると商品の数を返品された数分だけ増加
	...
	public List<ProductVO> todayProduct(Model model, @RequestParam("day") Date day) {
		...
		List<ProductVO> list = service.todayProduct(date);// 当日入った商品をホームページに表示するメソッド
